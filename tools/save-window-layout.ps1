...
.DESCRIPTION
  ...
  By itself this only RECORDS a placement; nothing enforces it. To have the autostart
  daemon (scripts\Start-WindowSlots.ps1) actively keep a window in its saved slot, open
  config\windows.toml afterwards and set `pin = true` on that [[windows]] entry.
...
.EXAMPLE
  .\tools\save-window-layout.ps1                    # capture every managed window
  .\tools\save-window-layout.ps1 -WhatIf             # preview without writing
  .\tools\save-window-layout.ps1 -Exe firefox.exe    # capture just one app
#>
[CmdletBinding()]
param(
    [string]$Exe,
